...
async function initDashboard() {
    ...
    if (profile) {
        ...
        if (document.getElementById('waShareBtn')) document.getElementById('waShareBtn').href = `https://wa.me/?text=${encodeURIComponent('Shop my latest collection here: https://' + fullLink)}`;
    }
...
async function loadRecentOrders() {
    ...
    if (!data || data.length === 0) {
        list.innerHTML = `<div class="text-center py-4"><i class="bi bi-inbox text-muted fs-2"></i><p class="text-muted mt-2 small">No orders yet.</p></div>`;
        return;
    }
    list.innerHTML = data.map(o => `
        <div class="order-row d-flex justify-content-between align-items-center py-2 border-bottom">
          <div><div class="fw-bold small">${o.id}</div><div class="text-muted" style="font-size:0.7rem;">${new Date(o.created_at).toLocaleDateString()}</div></div>
          <div class="text-end"><div class="fw-bold text-success small">₦${parseFloat(o.total_amount).toLocaleString()}</div><div class="badge-status status-${o.status}" style="font-size:0.6rem; padding:0.1rem 0.4rem; border-radius:4px;">${o.status}</div></div>
        </div>`).join('');
}
...
window.copyLink = function() {
    ...
    if (linkText && linkText !== 'Loading...') {
        navigator.clipboard.writeText(`https://${linkText}`).then(() => alert("Store link copied!"));
    }
};
...
// ─── 2. PRODUCT & INVENTORY LOGIC (ADD / LIST / DELETE) ───────────
window.saveProduct = async function(e) {
    ...
    try {
        ...
        await supabase.from('products').insert([{ 
            vendor_id: currentUser.id, title: document.getElementById('prodTitle').value, price: document.getElementById('prodPrice').value, 
    ...
    } catch (e) { alert(e.message); btn.disabled = false; btn.innerText = "Save Product"; }
};
...
window.deleteProduct = async function(id) { if(confirm("Delete permanently?")) { await supabase.from('products').delete().eq('id', id); window.loadProducts(); } };
window.copyProductLink = function(id) { navigator.clipboard.writeText(`https://myvendor.qzz.io/product/?vendor=${window.vendorSlug}&id=${id}`); alert("Link copied!"); };
...
window.clearEditImage = function(e) {
    ...
    currentExistingImageUrl = null; // Clear it so it deletes from DB on save
};
    ...
    list.innerHTML = orders.map(o => `
        <div class="order-card p-3 bg-white rounded-4 border mb-3" data-status="${o.status}">
            <div class="d-flex justify-content-between mb-2">
                <div><div class="fw-bold" style="font-size:0.9rem;">${o.id}</div><div class="text-muted" style="font-size:0.75rem;">${new Date(o.created_at).toLocaleDateString()}</div></div>
                <div class="text-end"><div class="fw-bold text-success" style="font-size:0.9rem;">₦${parseFloat(o.total_amount).toLocaleString()}</div><span class="badge-status status-${o.status}" style="font-size:0.65rem; padding:0.2rem 0.5rem; border-radius:4px; text-transform:uppercase;">${o.status}</span></div>
            </div>
            <div class="small mb-2" style="font-size:0.85rem;"><strong>${o.customer_name}</strong>: <span class="text-muted">${o.items}</span></div>
            ${o.rider_name ? `<div class="rider-info mt-2 p-2 rounded" style="background:#f0fdf4; border:1px dashed #bbf7d0; font-size:0.8rem;"><strong>Rider:</strong> ${o.rider_name} (${o.rider_phone})</div>` : ''}
...
window.filterOrders = function(status, pill) {
    ...
    document.querySelectorAll('.order-card').forEach(card => card.style.display = (status === 'all' || card.dataset.status === status) ? 'block' : 'none');
};

let currentOrderId = null;
window.openStatusModal = function(id, status) {
    currentOrderId = id; document.getElementById('statusSelect').value = status;
...
if(document.getElementById('statusSelect')) {
    document.getElementById('statusSelect').addEventListener('change', (e) => {
        const r = document.getElementById('riderDetailsGroup');
        if(r) r.style.display = e.target.value === 'shipped' ? 'block' : 'none';
    });
}
